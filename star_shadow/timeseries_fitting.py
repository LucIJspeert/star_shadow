...
def fit_multi_sinusoid_harmonics_per_group(times, signal, signal_err, p_orb, const, slope, f_n, a_n, ph_n, i_sectors,
    ...
    if verbose:
        ...
        resid = signal - model_linear - model_sinusoid
        bic = tsf.calc_bic(resid / signal_err, 1 + 2 * n_sect + 3 * n_sin + 2 * n_harm)
        print(f'Fit of harmonics - BIC: {bic:1.2f}. N_iter: {int(output.nit)}.')
    # update the parameters for each group
    for k, group in enumerate(f_groups):
        if verbose:
            print(f'Fit of group {k + 1} of {n_groups}', end='\r')
...
def fit_eclipse_empirical_sinusoids(times, signal, signal_err, p_orb, timings, cubic_pars, const, slope, f_n, a_n, ph_n,
    ...
    for k, group in enumerate(f_groups):
        ...
        if verbose:
            ...
            print(f'Fit of group {k + 1} of {n_groups} - BIC: {bic:1.2f}. N_iter: {int(output.nit)}.')
...
def fit_eclipse_physical_sinusoid(times, signal, signal_err, p_orb, t_zero, ecl_par, const, slope,
    ...
    for k, group in enumerate(f_groups):
        ...
        # separate results
        n_sin_g = len(res_freqs[group])
        res_ecl_par = output.x[0:6]
        res_const = output.x[6:6 + n_sect]
        res_slope = output.x[6 + n_sect:6 + 2 * n_sect]
        out_freqs = output.x[6 + 2 * n_sect:6 + 2 * n_sect + n_sin_g]
        out_ampls = output.x[6 + 2 * n_sect + n_sin_g:6 + 2 * n_sect + 2 * n_sin_g]
        out_phases = output.x[6 + 2 * n_sect + 2 * n_sin_g:6 + 2 * n_sect + 3 * n_sin_g]
        res_freqs[group] = out_freqs
        res_ampls[group] = out_ampls
        res_phases[group] = out_phases
        if verbose:
