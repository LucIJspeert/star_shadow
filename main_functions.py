...
def eclipse_analysis_elements(p_orb, t_zero, timings_tau, depths, bottom_dur, p_err, timing_errs, depths_err, f_h, a_h,
    ...
    if verbose:
        ...
        print(f'\033[0;32;48me: {e:.{rnd_e}f} (+{e_err[1]:.{rnd_e}f} -{e_err[0]:.{rnd_e}f}), '
              ...
              f'phi_0: {phi_0:.{rnd_phi0}f} '
              f'(+{phi_0_err[1]:.{rnd_phi0}f} -{phi_0_err[0]:.{rnd_phi0}f}), '
              f'bounds ({phi_0_bds[0]:.{rnd_phi0}f}, {phi_0_bds[1]:.{rnd_phi0}f}), \n'
...
def eclipse_analysis(tic, times, signal, signal_err, i_sectors, save_dir, data_id=None, overwrite=False, verbose=False):
    ...
    # errors_12, bounds_12, formal_errors_12, dists_in_12, dists_out_12 = out_12[9:]
...
def pulsation_analysis_disentangle(times, signal, p_orb, t_zero, const, slope, f_n, a_n, ph_n, par_ellc,
    ...
    if os.path.isfile(file_name) & (not overwrite):
        ...
        if file_name is not None:
            ut.save_results_disentangle(const_r, f_n_r, a_n_r, ph_n_r, file_name, data_id)
    # make the models
    harmonics, harmonic_n = af.find_harmonics_from_pattern(f_n, p_orb)
    non_harm = np.delete(np.arange(len(f_n)), harmonics)
